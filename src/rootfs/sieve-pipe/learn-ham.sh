...
if [ -n "$DOVECOT_RSPAMD_FUZZY_DENIED_TAG" ]; then
  rspamc $rspamcArgs -f "$DOVECOT_RSPAMD_FUZZY_DENIED_TAG" fuzzy_del "$tmpFile"
fi
if [ -n "$DOVECOT_RSPAMD_FUZZY_WHITE_TAG" ]; then
  rspamc $rspamcArgs -f "$DOVECOT_RSPAMD_FUZZY_WHITE_TAG" fuzzy_add "$tmpFile"
fi

rspamc $rspamcArgs learn_ham "$tmpFile"
