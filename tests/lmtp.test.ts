...
describe("lmtp", () => {
	...
	const { imapPort, lmtpPort } = useContainer();

	it("delivers message to valid user — message appears in INBOX", async () => {
	...
	it("rejects delivery to unknown user with 5xx", async () => {
	...
	it("rejects delivery to sendonly user with 5xx", async () => {
		const { rcptResponse } = await deliverMessage(lmtpPort, {
			from: FROM,
	...
	it("global-spam.sieve: message with X-Spam: YES is filed into Junk, not INBOX", async () => {
		const { rcptResponse, dataResponse } = await deliverMessage(lmtpPort, {
			from: FROM,
			to: "spamtest@example.com",
